                          ...
                          uint32_t noise_var)
{
  ...
  // Multiply matrix inverse by rx signal vector: rxdataF_zforcing = inv_H_h_H * rxdataF_comp
  // ctx=0 writes directly into rxdataF_zforcing[rtx], avoiding memset + outtemp on first iteration
  c16_t outtemp[12 * nb_rb_0] __attribute__((aligned(32)));
  c16_t rxdataF_zforcing[nl][12 * nb_rb_0];
  const int mmse_shift = shift - (fp_flag == 1 ? 1 : 0);

  for (int rtx = 0; rtx < nl; rtx++) {
    mult_complex_vectors(inv_H_h_H[0][rtx], rxdataF_comp[symbol][0], rxdataF_zforcing[rtx], sizeofArray(outtemp), mmse_shift);
    for (int ctx = 1; ctx < nl; ctx++) {
      mult_complex_vectors(inv_H_h_H[ctx][rtx], rxdataF_comp[symbol][ctx], outtemp, sizeofArray(outtemp), mmse_shift);
      nr_a_sum_b(rxdataF_zforcing[rtx], outtemp, nb_rb_0);
    }
#ifdef DEBUG_DLSCH_DEMOD
