...
static __attribute__((unused)) int32_t ceil_div_pos_i32(int32_t num, int32_t den)
{
    if (den <= 0)
        return 0;
...
static inline __attribute__((unused)) int32_t p7_max_i32(int32_t a, int32_t b)
{
    return a > b ? a : b;
}
 ...
 */
static inline __attribute__((unused)) int p7_ewma_effectively_zero_i32(
    int32_t value,
    int32_t denom)
{
    if (value <= 0)
    ...
    const vnf_timing_stats_t *stats)
{
	...
	/* slot_ahead is read from the autonomous timing thread; access it atomically. */
	if (__atomic_load_n(&p7_info->slot_ahead, __ATOMIC_RELAXED) <= 0)
		__atomic_store_n(&p7_info->slot_ahead, 4, __ATOMIC_RELAXED);
	...
	int32_t target = __atomic_load_n(&p7_info->slot_ahead, __ATOMIC_RELAXED);
	...
	/* ===== Apply adjustment ===== */
	int32_t cur_slot_ahead = __atomic_load_n(&p7_info->slot_ahead, __ATOMIC_RELAXED);
	if (target != cur_slot_ahead) {
		int32_t delta = target - cur_slot_ahead;
		/* Compensate EWMA mean for the shift in reference frame */
		p7_info->estimated_mean_late -= delta * sd;
		__atomic_store_n(&p7_info->slot_ahead, target, __ATOMIC_RELAXED);
		p7_info->last_adjustment_sfn = p7_info->sfn;
		p7_info->last_adjustment_slot = p7_info->slot;
	}
}
