...
static void test_missed_deadline_preserves_timeline(void)
{
  nfapi_pnf_pacer_t pacer = {0};
  struct timespec deadline = {0};
  ...
  assert_timespec(pacer.next_slot, 0, 1000000L);

  assert(nfapi_pnf_pacer_next(&pacer, 0, (struct timespec){.tv_nsec = 1600000L}, &deadline));
  assert_timespec(deadline, 0, 2000000L);
}
...
int main(void)
{
  ...
  test_missed_deadline_preserves_timeline();
